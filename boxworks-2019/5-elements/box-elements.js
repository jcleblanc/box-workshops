...
app.get('/', (req, res) => {
  const scopes = ['base_explorer', 'root_readwrite', 'item_preview', 'item_download', 'item_rename', 'item_share', 'item_delete'];
  const folder = 'https://api.box.com/2.0/folders/76451036177';
  ...
  client.exchangeToken(scopes, folder).then((tokenInfo) => {
    ...
    res.render('contentExplorer', { at: tokenInfo.accessToken, fid: '76451036177' });
  }).catch((err) => {
